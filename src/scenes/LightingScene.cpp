...
void LightingScene::open() {
    ...
    en::Actor ambientLight = engine.makeActor("AmbientLight");
    ...
    if (AnimateLightProperties) {
        ambientLight.add<LightPropertyAnimator>();
    }
    ...
    if (AnimateLightProperties) {
        directionalLight.add<LightPropertyAnimator>();
    }

    en::Actor spotLight = engine.makeActor("SpotLight");
    spotLight.add<en::Transform>().move({0, 0, 2});
    {
        const glm::vec3 spotLightColor = {0, 1, 1};

        {
            en::Actor child = engine.makeActor("SpotLightModel");
            ...
            const auto model = en::Models::get(config::MODEL_PATH + "cone_smooth.obj");
            const auto material = std::make_shared<en::Material>("color");
    ...
    if (AnimateLightProperties) {
        spotLight.add<LightPropertyAnimator>();
    }

    en::Actor rotatingLights = engine.makeActor("RotatingLights");
    rotatingLights.add<en::Transform>();
    rotatingLights.add<RotatingBehavior>(glm::vec3(0,1,0));

    const auto lightModel = en::Models::get(config::MODEL_PATH + "sphere.obj");
    for (int i = 0; i < NumRotatingLights; ++i) {

        en::Actor light = engine.makeActor("Light");

        {
            const float rotationAngle = glm::two_pi<float>() * (i + 1.f) / NumRotatingLights;
            light.add<en::Transform>()
                .setParent(rotatingLights)
                .move(glm::rotate(glm::vec3(0, 0, 2.5f), rotationAngle, glm::vec3(0, 1, 0)))
                .scale(glm::vec3(0.1f));
        }

        const glm::vec3 lightColor = glm::abs(glm::make_vec3(glm::circularRand(1.f)));
        ...
        if (AnimateLightProperties) {
            light.add<LightPropertyAnimator>();
        }
    }

    en::Actor sphere = engine.makeActor("Sphere");
    sphere.add<en::Transform>().move({0, 0, 0});
    {
        const auto model = en::Models::get(config::MODEL_PATH + "sphere.obj");
    ...
    en::Actor pillar = engine.makeActor("Pillar");
    pillar.add<en::Transform>().move({-4, -1, 0}).scale(glm::vec3(0.01f));
    {
        const auto model = en::Models::get(config::MODEL_PATH + "Pillar_tall.obj");
        ...
        pillar.add<en::RenderInfo>(model, std::move(material));
    }

    en::Actor plane = engine.makeActor("Plane");
    plane.add<en::Transform>().move({0, -1, 0}).setLocalScale(glm::vec3(7));
    {
        const auto model = en::Models::get(config::MODEL_PATH + "plane.obj");
        auto material = std::make_shared<en::Material>("pbr");
