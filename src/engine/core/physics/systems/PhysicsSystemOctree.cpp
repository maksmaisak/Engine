...
std::tuple<bool, float> PhysicsSystemOctree::move(Entity entity, Transform& tf, Rigidbody& rb, float dt) {
    ...
    Bounds3D bounds = m_octreeRoot.getBounds().clamp(rb.collider->getBounds());
...
void PhysicsSystemOctree::removeInvalidEntitiesFromTree() {
    ...
    const auto isInvalidEntity = [this](Entity e, const Bounds3D& bounds) -> bool {
...
void PhysicsSystemOctree::updateTree(Entity entity, const Rigidbody& rb, const Transform& tf) {
    ...
    const Bounds3D bounds = m_octreeRoot.getBounds().clamp(rb.collider->getBounds());
    const Bounds3D* oldBounds = m_previousBounds.tryGet(entity);
    if (!oldBounds)
...
void PhysicsSystemOctree::draw() {
    ...
    addToVolumeRenderer = [&](const OctreeNode& node) {
        ...
        const Bounds3D& bounds = node.getBounds();
